...
import sigmod2021.esp.api.expression.predicate.spatial.CoveredBy;
import sigmod2021.esp.expressions.arithmetic.EVSpatialExpression;
import sigmod2021.esp.expressions.predicate.EVPredicate;
import sigmod2021.esp.expressions.predicate.spatial.EVSpatialCoveredBy;
 ...
 */
@MetaInfServices
public class CoveredByTranslator extends Translator<CoveredBy, EVPredicate> {

    public CoveredByTranslator() {
        super(CoveredBy.class, EVPredicate.class);
    }

    @Override
    protected EVPredicate process(CoveredBy in, ExpressionTranslator translator, EventSchema schema, Bindings bindings)
        ...
        if (gct == DataType.GEOMETRY) {
            ...
            return new EVSpatialCoveredBy(tl, tr);
        } else
            throw new IncompatibleTypeException("No implementation for Covered By with input-types: " + lt + " and " + rt);
    }
}
