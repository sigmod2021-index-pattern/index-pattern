...
import sigmod2021.esp.api.expression.predicate.spatial.Intersects;
import sigmod2021.esp.expressions.arithmetic.EVSpatialExpression;
import sigmod2021.esp.expressions.predicate.EVPredicate;
import sigmod2021.esp.expressions.predicate.spatial.EVSpatialIntersects;
 ...
 */
@MetaInfServices
public class IntersectsTranslator extends Translator<Intersects, EVPredicate> {

    public IntersectsTranslator() {
        super(Intersects.class, EVPredicate.class);
    }

    @Override
    protected EVPredicate process(Intersects in, ExpressionTranslator translator, EventSchema schema, Bindings bindings)
        ...
        if (gct == DataType.GEOMETRY) {
            ...
            return new EVSpatialIntersects(tl, tr);
        } else
            throw new IncompatibleTypeException("No implementation for Intersects with input-types: " + lt + " and " + rt);
    }
}
