...
import sigmod2021.esp.api.expression.predicate.spatial.Covers;
import sigmod2021.esp.expressions.arithmetic.EVSpatialExpression;
import sigmod2021.esp.expressions.predicate.EVPredicate;
import sigmod2021.esp.expressions.predicate.spatial.EVSpatialCovers;
...
@MetaInfServices
public class CoversTranslator extends Translator<Covers, EVPredicate> {

    public CoversTranslator() {
        super(Covers.class, EVPredicate.class);
    }

    @Override
    protected EVPredicate process(Covers in, ExpressionTranslator translator, EventSchema schema, Bindings bindings)
        ...
        if (gct == DataType.GEOMETRY) {
            ...
            return new EVSpatialCovers(tl, tr);
        } else
            throw new IncompatibleTypeException("No implementation for Covers with input-types: " + lt + " and " + rt);
    }
}
