...
import sigmod2021.esp.api.expression.predicate.spatial.Overlaps;
import sigmod2021.esp.expressions.arithmetic.EVSpatialExpression;
import sigmod2021.esp.expressions.predicate.EVPredicate;
import sigmod2021.esp.expressions.predicate.spatial.EVSpatialOverlaps;
 ...
 */
@MetaInfServices
public class OverlapsTranslator extends Translator<Overlaps, EVPredicate> {

    public OverlapsTranslator() {
        super(Overlaps.class, EVPredicate.class);
    }

    @Override
    protected EVPredicate process(Overlaps in, ExpressionTranslator translator, EventSchema schema, Bindings bindings)
        ...
        if (gct == DataType.GEOMETRY) {
            ...
            return new EVSpatialOverlaps(tl, tr);
        } else
            throw new IncompatibleTypeException("No implementation for Overlaps with input-types: " + lt + " and " + rt);
    }
}
