...
import sigmod2021.esp.expressions.predicate.numeric.EVNumericLessEq;
import sigmod2021.esp.expressions.predicate.string.EVStringLessEq;
...
@MetaInfServices
public class LessEqTranslator extends Translator<LessEq, EVPredicate> {

    public LessEqTranslator() {
        super(LessEq.class, EVPredicate.class);
    }

    @Override
    protected EVPredicate process(LessEq in, ExpressionTranslator translator, EventSchema schema, Bindings bindings)
        ...
        if (gct.isNumeric()) {
            ...
            return new EVNumericLessEq(gct, tl, tr);
            ...
            return new EVStringLessEq(tl, tr);
        } else
            throw new IncompatibleTypeException("No implementation for LessEq with input-types: " + lt + ", " + rt);
    }
}
