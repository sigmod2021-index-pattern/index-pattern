...
import sigmod2021.esp.api.expression.predicate.spatial.Equals;
import sigmod2021.esp.expressions.arithmetic.EVSpatialExpression;
import sigmod2021.esp.expressions.predicate.EVPredicate;
import sigmod2021.esp.expressions.predicate.spatial.EVSpatialEquals;
 ...
 */
@MetaInfServices
public class EqualsTranslator extends Translator<Equals, EVPredicate> {

    public EqualsTranslator() {
        super(Equals.class, EVPredicate.class);
    }

    @Override
    protected EVPredicate process(Equals in, ExpressionTranslator translator, EventSchema schema, Bindings bindings)
        ...
        if (gct == DataType.GEOMETRY) {
            ...
            return new EVSpatialEquals(tl, tr);
        } else
            throw new IncompatibleTypeException("No implementation for Equals with input-types: " + lt + " and " + rt);
    }
}
